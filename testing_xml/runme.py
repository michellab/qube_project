...
lig1 = system1.molecules().first()[0] 

mol = lig1.edit().setProperty("forcefield", ffToProperty("qube")).commit()
system1.update(mol)
lig1 = system1.molecules().first()[0] 

lig1.hasProperty("forcefield")
print("Lig1 properties:")
print(lig1.properties())
bss_lig1 = BSS._SireWrappers.Molecule(lig1)

xmlfile2 = "MOL2.xml"
pdbfile2 = "MOL_modified.pdb"
(molecules2, space) = assignVirtualSites(pdbfile2, xmlfile2)
system2 = createSystem(molecules2)
system2 = setupForcefields(system2, space)
lig2 = system2.molecules().first()[0] 

mol = lig2.edit().setProperty("forcefield", ffToProperty("qube")).commit()
system2.update(mol)
lig2 = system2.molecules().first()[0] 

print("Lig2 properties:")
print(lig2.properties())
bss_lig2 = BSS._SireWrappers.Molecule(lig2)

mappings, scores = BSS.Align.matchAtoms(bss_lig1, bss_lig2, matches=10,  return_scores=True, scoring_function="RMSDalign")   
mapping = mappings[0]
inverted_mapping = dict([[v,k] for k,v in mapping.items()])
bss_lig2 = BSS.Align.rmsdAlign(bss_lig2, bss_lig1, inverted_mapping)
merged = BSS.Align.merge(bss_lig1, bss_lig2, mapping, allow_ring_breaking=False, allow_ring_size_change=False)
merged
bss_sys1 = BSS._SireWrappers.System(system1)
bss_sys1.removeMolecules(bss_lig1)
bss_sys1.addMolecules(merged)


# Log the mapping used
writeLog(bss_lig1, bss_lig2, mapping)
...
process = BSS.Process.Somd(bss_sys1, protocol)
process.getOutput()
